...
"""
Usage:
    run.py run      [-m] [-s=STEP] [-c=CELLTYPE] [--] <SIMTIME>
                    (<BCL> | <BSTART> <BSTOP> [<BSTEP>])
    run.py s1s2     [-s=STEP] [-c=CELLTYPE] <BCL> <BEATS> <S1S2SPAN>
    run.py burst    [-s=STEP] [-c=CELLTYPE] [--]
                    <BCL> <BEATS> <BURST_BCL> <BURST_BEATS> [<REST_BCL> <REST_BEATS>]
    run.py freq     [-m] [-s=STEP] [-c=CELLTYPE] [--] <SIMTIME>
                    (<FREQUENCY> | (<FSTART> <FSTOP>) [<FSTEP>])
    run.py plot     (--figtable | ([--save] (<ChapterNo> <FigureNo> | <FigNoinPaper>)))
    run.py <command> [<args>...]
    run.py          (-h | --help)
...
from util_bundle.util import clean_result_for_plot, bar_arrange

CELL_MODEL = ''  # assigned in main()
...
measure.usage = \
    """
usage: run.py measure [-m] (-d <DIR> | <FILE>...)
...
def quickplot(args):
    ...
    if args['-x']:
        xlim = (float(args['<XSTART>']), float(args['<XEND>']))
    else:
        xlim = None
...
usage: run.py quickplot [-VI] [-o=OUT]
                        [(-x <XSTART> <XEND>)]
                        (<FILE> <LABEL>)...
...
def cleanresult(args):
    # print(args)
    for file_ in args['<FILE>']:
        clean_result_for_plot(file_, add_underline=args['--underline'],
                              truncate_to=int(args['--truncate']),
                              reset_start_time=args['--reset-time'])


cleanresult.usage = \
    """
usage: run.py cleanresult [-ru] [-t num] <FILE>...
...
def plot(args):
    ...
    if c_num == 2 and f_num == 20:
        data = []
        for f in get_files(1, _measure=True):
    ...
    # Figure 2.21 #
        ...
        f_ven_measures = get_files(0, _measure=True)
        f_at_measures = get_files(1, _measure=True)
    ...
    # Fig 10, Ca2+ handling #
        ...
        for i, _data in enumerate(data_amp):
            if data_amp[i][2] == 'Model':  # has a revert color scheme
                ...
                if data_amp[i][2] != 'Walden et al.':
                    ...
                    axe.annotate(data_amp[i][2], xy=(x_pos[0][i] + 0.15, 0.05), xycoords='axes fraction',
                                 rotation='vertical', va='bottom', ha='center')
            axe.errorbar(x_pos[0][i], data_amp[i][0], yerr=data_amp[i][1], color='k', capthick=1.5)
        ...
        axe.set_xlim([0, 1])
        axe.set_xticks([])
        axe.set_ylabel("Fractional Ca" + r"$^{2+}$" + " Release")
        ...
        bcl = [200, 500, 1000]
        for _bcl in bcl:
            str_atr_data = "output/out_atria_WT/currents_bcl-" + str(_bcl) + "_ISO-0.dat"
            ...
            _d[:, i_t] = _d[:, i_t] - 28000
            _d[:, i_t] = _d[:, i_t] - 2000 + 2 * _bcl

            axe.plot(_d[:, i_t], _d[:, i_ca_i], 'k', label="BCL " + str(_bcl))
    ...
    # FigE, Ca traces with Caffeine #
        ...
        if c_num is not None:
            print('Figure %d.%d has not been reproduced. Choose another figures please.'
                  % (c_num, f_num))
        else:
            print('Figure %d has not been reproduced. Choose another figure please.'
                  % f_num_inpaper)
        return
...
def main():
    ...
    #                  '<BURST_BCL>': Or(None, Use(int)),
    #                  '<BURST_BEATS>': Or(None, Use(int)),
    #                  '<REST_BCL>': Or(None, Use(int)),
    ...
    # def validate(args_):
    #     try:
    #         args_ = schema.validate(args_)
    #         return args_
    #     except SchemaError as e:
    #         exit(e)
    #
    # args = validate(docopt(__doc__))
    ...
    if args['freq']:
        ...
        burst(args)
    elif args['<command>'] == 'cleanresult':
        cleanresult(docopt(cleanresult.usage))
    elif args['<command>'] == 'quickplot':
        quickplot(docopt(quickplot.usage))
    else:
        sys.stderr.write("Error! Don't know the subcommand: " + args['<command>'] + '\n')
